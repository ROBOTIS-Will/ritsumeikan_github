...
より良いナビゲーションのためには、costmapの計算が必要です。costmapはロボットの位置と方向を表すpose情報とセンサー値、障害物情報、SLAMによって得られた地図に基づいて計算することができます。計算された結果は、ロボットが衝突する領域、衝突可能性がある領域、自由に移動可能な領域などで表され、これらの計算結果をもとに、ロボットの安全な移動経路の生成を助けます。costmapは2種類に分類することができ、ロボットのスタート位置から目的地までの経路計画を策定するために使用されるglobal costmapと、ロボットの周辺において障害物などを回避するための安全な経路を計算するlocal costmapがあります。ROSでこのようなcostmapは値で表されますが、0はロボットが自由に移動できる領域で、255に近づくほどロボットの衝突が発生する可能性の高い領域として区分されます。
...
1. ロボットの制御位置(dx、dy、dtheta)を個別に生成します。
...
## ナビゲーションノードの実行 
### [Remote PC] ロスコアを実行してください。 
  ```bash
  $ roscore
  ```
### [TurtleBot PC] タートルボット３の応用プログラムを始めるための基本パッケージを持ってきてください。 
  ```bash
...
Navigationを行う際に最も重要なことは、ロボットの正確な現在地を地図上に示すことです。ロボットのエンコーダとIMU、LDSなど各種センサーから得られた情報をもとにTurtleBot3の位置を予測するには、確率をベースにしたAMCL(Adaptive Monte Carlo Localization)というparticle filterが使用されます。AMCLは、センサーの情報に基づいてロボットが存在する可能性がある位置を予測しますが、アルゴリズムのパラメータ値に設定された移動量をロボットが移動するたびに、このような予測された位置の値を更新し、更新が繰り返されるたびに予測された位置の値の誤差が減少することになります。

[Remote PC]ロボットがNavigationを実行するための地図が実行され、ロボットのLDSセンサーが正常に実行されれば、RVizの`2D Pose Estimate`ボタンを使用して、地図上のロボットの位置をロボットの実際の位置と方向に合わせて設定する必要があります。`2D Pose Estimate`ボタンをクリックして、ロボットが実際に位置している点をRVizの地図上でクリックし、ロボットが向いている方向にマウスをドラッグすると、大きな緑色の矢印の方向をロボットが向いている方向に設定することができます。LDSセンサーの値と地図で表示された障害物の位置が一致するほど正確なNavigationが可能なため、初期位置を設定するための手順を繰り返し実行し、なるべく正確な位置に設定する必要があります。以下の手順で、ロボットの位置を設定します。
...
> 上の図で、ロボットの周辺に広く表示された多数の小さな緑の矢印は、ロボットの初期位置を指定した際にロボットのセンサーと各種情報を介してAMCLが計算したロボットの現在位置です。ロボットが移動するに従って分布していた現在地の予想値がロボットに向かって近づき、収束する様子が分かります。
...
[Remote PC] RVizで地図が表示され、ロボットの初期位置が正しく設定されると、ロボットが到着目的地の位置と方向を指定することができます。RVizの`2D Nav Goal`ボタンをクリックし、ロボットが移動できる目的地をクリックし、ロボットが向いている方向をドラッグして、矢印の方向を指定します。
...
ロボットの初期位置と方向を設定します。RVizの2D Pose Estimateボタンで、ロボットのPoseを手動で設定できます。
...
ロボットのx、y、theta方向に、加速度の限界値をm/s^2とrad/s^2単位で設定します。
...
### xy_goal_tolerance 
ロボットが指定された目的地に到着した際の、x、y座標上の距離誤差を設定します。
 
### yaw_goal_tolerance 
ロボットが指定された目的地に到着した際の、ロボットが向く角度の誤差を設定します。
